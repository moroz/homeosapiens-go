...
func eventCard(localizer *i18n.Localizer, e *services.EventListDto) Node {
	return Article(
		...
		Header(
			Span(
				...
				Iff(e.VenueID.Valid, func() Node {
					return Text(
						fmt.Sprintf("%s, %s", *e.VenueCityEn, helpers.TranslateCountry(localizer, *e.VenueCountryCode)),
					)
				}),
